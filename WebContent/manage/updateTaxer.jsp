		...
		<div class="content">
					...
					<tbody>
						<tr>
							...
							<td class="kv-content"><input type="text" name="taxerCode"
								placeholder="纳税人识别号" class="easyui-validatebox"  readonly="readonly"
								data-options="required:true" value="${taxer.taxerCode}" title="不可修改"><span></span></td>
							<td class="kv-label">办税专员姓名</td>
							<td class="kv-content"><input type="text" name="taxerName"
								placeholder="纳税人名称" class="easyui-validatebox"  readonly="readonly"
								data-options="required:true" value="${taxer.taxerName}" title="不可修改"><span></span></td>
						</tr>
						<tr>
							<td class="kv-label">手机号</td>
							<td class="kv-content"><input type="text" name="mobile"
								placeholder="手机号" class="easyui-validatebox" 
								data-options="required:true" value="${taxer.mobile}"><span></span></td>
							<td class="kv-label">地址</td>
							<td class="kv-content"><input type="text" name="address"
								placeholder="地址" class="easyui-validatebox" 
								data-options="required:true" value="${taxer.address}"><span></span></td>
						</tr>
						<tr>
							<td class="kv-label">邮箱</td>
							<td class="kv-content"><input type="text" name="email"
								placeholder="邮箱" class="easyui-validatebox" 
								data-options="required:true,validType:'email'" value="${taxer.email}"><span></span></td>
						...
						<tr>
							...
							<td class="kv-content"><input class="easyui-validatebox" data-options="required:true"  type="date" name="birthday"
								placeholder="生日" style="width: 210px;height: 30px"
								value="${taxer.birthday}"><span></span></td>
			...
			<div class="btn-selection">
				<a href="javascript:void(0);" iconcls="icon-ok" class="easyui-linkbutton save-btn"
					data-options="selected:true">修改</a> <a href="javascript:void(0);"
...
$.extend($.fn.validatebox.defaults.rules, {
	...
	$(function() {
		...
		$.post("getOrganServlet.do", {}, function(result) {
			...
			$("select:eq(1)").val('${taxer.organId}')
		}, "json")

		$("select:eq(0)").val('${taxer.sex}')
		...
		$("a:eq(0)").bind("click", function() {
			...
			$.post("updateTaxer",$("form").serialize(), function(result) {
